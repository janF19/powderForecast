...
from freeride.config import DEM_DIR, utm_epsg
...
def fetch_copernicus(resort_key, bbox, center_lonlat):
    """Clip Copernicus 30m to bbox, reproject to local UTM, return GeoTIFF path."""
    safe_key = resort_key.replace("/", "_").replace(" ", "_")[:60]
    dest = DEM_DIR / f"{safe_key}_cop30.tif"
    if dest.exists():
